...
def preprocess_features(
    ...
    # Categorical features — fill NaN with 0 and cast to int
    user_id = df["userId"].fillna(0).astype(np.int64).values
    mount_ns = df["mountNamespace"].fillna(0).astype(np.int64).values
    event_id = df["eventId"].fillna(0).astype(np.int64).values
